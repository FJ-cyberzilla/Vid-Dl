...
async def test_check_binary_not_found(aria2c_client: Aria2cClient) -> None:
    with patch("shutil.which", return_value=None):
        with pytest.raises(Aria2cNotFoundError):
...
def test_build_command(aria2c_client: Aria2cClient) -> None:
    ...
    cmd = aria2c_client._build_command("https://url", Path("/tmp/out.mp4"), options)
...
async def test_run_aria2c_success(aria2c_client: Aria2cClient, tmp_path: Path) -> None:
    with patch("asyncio.create_subprocess_exec") as mock_exec:
        ...
        progress = []
        options = Aria2cOptions(progress_callback=lambda p: progress.append(p), retries=1)
...
async def test_download_full_cycle(aria2c_client: Aria2cClient, tmp_path: Path) -> None:
    ...
    with patch.object(aria2c_client, "_check_binary", new_callable=AsyncMock):
        with patch.object(aria2c_client, "_run_aria2c", new_callable=AsyncMock) as mock_run:

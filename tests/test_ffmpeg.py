...
def test_parse_time() -> None:
    assert _parse_time("frame=  100 fps= 25 q=-1.0 Lsize=    1000kB time=00:00:04.00 bitrate=2000.0kbits/s speed=1.0x") == 4.0
...
async def test_check_ffmpeg_not_found(ffmpeg_proc: FFmpegProcessor) -> None:
    with patch("asyncio.create_subprocess_exec", side_effect=FileNotFoundError):
        with pytest.raises(FFmpegNotFoundError, match="FFmpeg binary not found"):
            await ffmpeg_proc.check_ffmpeg()
...
async def test_run_ffmpeg_success(ffmpeg_proc: FFmpegProcessor) -> None:
    with patch("asyncio.create_subprocess_exec") as mock_exec:
        ...
        await ffmpeg_proc._run_ffmpeg(["ffmpeg", "-i", "in.mp4", "out.mp4"], progress_callback=callback)
...
def test_sync_wrappers(ffmpeg_proc: FFmpegProcessor, tmp_path: Path) -> None:
    ...
    with patch.object(ffmpeg_proc, "merge_audio_video", new_callable=AsyncMock) as mock_async:
        ffmpeg_proc.merge_audio_video_sync(v, a, out)
        mock_async.assert_called_once()

    with patch.object(ffmpeg_proc, "extract_thumbnail", new_callable=AsyncMock) as mock_async:

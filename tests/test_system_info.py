...
def test_temperatures_fallback(tmp_path: Path) -> None:
    ...
    with patch("infrastructure.system._PSUTIL_AVAILABLE", False):
        # Patch Path("/sys/class/thermal") inside get_temperatures to use our tmp_path thermal_dir
        with patch("infrastructure.system.Path") as mock_path_cls:
...
def test_full_report() -> None:
    # Mock psutil.swap_memory if available to avoid Permission denied on /proc/vmstat under Android
    from infrastructure.system import _PSUTIL_AVAILABLE
    if _PSUTIL_AVAILABLE:
        with patch("psutil.swap_memory") as mock_swap:
            mock_swap.return_value = MagicMock(total=1000, used=200, free=800, percent=20.0)

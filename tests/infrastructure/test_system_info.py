...
def test_environment_detection() -> None:
    ...
    with patch("sota_dl.infrastructure.system_monitor.is_linux", return_value=True):
...
def test_cpu_usage_no_psutil() -> None:
    with patch("sota_dl.infrastructure.system_monitor._PSUTIL_AVAILABLE", False):
        assert SystemInfo.get_cpu_usage() == -1.0
        assert SystemInfo.get_per_cpu_usage() == []


def test_cpu_usage_with_psutil() -> None:
    with patch("sota_dl.infrastructure.system_monitor._PSUTIL_AVAILABLE", True):
...
def test_temperatures_fallback(tmp_path: Path) -> None:
    ...
    with (
        patch("sota_dl.infrastructure.system_monitor._PSUTIL_AVAILABLE", False),
        patch("sota_dl.infrastructure.system_monitor.Path") as mock_path_cls,
    ):
...
def test_full_report() -> None:
    ...
    from sota_dl.infrastructure.system import _PSUTIL_AVAILABLE

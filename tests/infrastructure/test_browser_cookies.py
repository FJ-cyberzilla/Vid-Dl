...
def test_get_cookies_for_url_dynamic_success() -> None:
    with patch("browser_cookie3.chrome") as mock_chrome:
        ...
        cookies = BrowserCookieAdapter.get_cookies_for_url("https://example.com")

        assert cookies == {"cookie1": "value1", "cookie2": "value2"}
        mock_chrome.assert_called_once_with(domain_name="https://example.com")
...
def test_get_cookies_for_url_dynamic_failure_fallback_success(
    ...
    # Patch DATA_DIR
    monkeypatch.setattr(
        "sota_dl.infrastructure.adapters.browser_cookies.DATA_DIR", tmp_path
    )
...
def test_load_cookies_from_file_valid(tmp_path: Path) -> None:
    ...
    cookies = BrowserCookieAdapter._load_cookies_from_file(cookie_file)
